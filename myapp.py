...
st.set_page_config(page_title="Smart Grocery Basket Optimizer", layout="wide")
st.title("🛒 Smart Grocery Basket Optimizer & Price Predictor")
st.caption("Budget-aware nutritional basket optimization + commodity price forecasting")
...
# ---------------- Greedy Basket Planner ----------------
picked_idx = []
for cat in available["Category"].unique():
    cat_rows = available[available["Category"] == cat]

    # If user wants non-veg and this is the Protein group, prioritize cheapest non-veg
    if diet_pref == "non-veg" and cat == "Protein" and (cat_rows["Type"] == "Non-Veg").any():
        cat_rows = cat_rows[cat_rows["Type"] == "Non-Veg"]
...
if spent > budget:
    ...
    remaining["Efficiency"] = (
                                      remaining["Calories"] / max(household_calorie_goal, 1) + remaining[
                                  "Protein"] / max(household_protein_goal, 1)
                              ) / remaining["Price"]
...
col1.metric("Basket Cost", f"Rs {meal_cost:.2f}")
...
st.subheader("Recommended Shopping Basket")
...
st.download_button("Download Shopping Basket (CSV)", csv_bytes, "my_shopping_basket.csv", "text/csv")
...
# ---------------- Zoomed Window Definition ----------------
RECENT_WEEKS = 26  # Shows the most recent 26 weeks (~6 months)
recent_grocery = grocery_data.tail(RECENT_WEEKS)

st.subheader("Price Forecast for Items in Your Basket")
if tracked_plan_items:
    ...
    for ax, item in zip(axes, tracked_plan_items):
        ax.plot(recent_grocery["Week"], recent_grocery[item], marker="o", markersize=4, label="Actual")
        ax.plot(future_df["Week"], future_df[item], marker="o", markersize=6, linestyle="--", color="red",
                label="Forecast")
        ax.set_title(f"{item} ({model_names[item]})", fontsize=10)
    ...
    st.write("None of this week's recommended items have historical price series to chart.")
st.caption(f"Showing the last {RECENT_WEEKS} weeks + forecast.")

with st.expander(f"View forecast charts for all {len(item_cols)} tracked items"):
    ...
    for ax, item in zip(axes, item_cols):
        ax.plot(recent_grocery["Week"], recent_grocery[item], marker="o", markersize=3, label="Actual")
        ax.plot(future_df["Week"], future_df[item], marker="o", markersize=6, linestyle="--", color="red",
                label="Forecast")
        ax.set_title(f"{item} ({model_names[item]})", fontsize=10)
        ax.set_xlabel("Week", fontsize=8)
...
# ---------------- Zoomed Basket Cost Forecast ----------------
st.subheader("Your Basket's Weekly Cost Trend & Forecast")
...
st.metric(f"Predicted basket cost, week {int(future_df['Week'].iloc[-1])}", f"Rs {predicted_plan_total:.2f}")

fig, ax = plt.subplots(figsize=(10, 4.5))
recent_plan_hist_cost = plan_hist_cost.tail(RECENT_WEEKS)
recent_weeks_x = recent_grocery["Week"]

ax.plot(recent_weeks_x, recent_plan_hist_cost, marker="o", markersize=4, label="Actual (tracked items)")
ax.plot(future_df["Week"], plan_future_cost, marker="o", markersize=6, linestyle="--", color="red", label="Forecast")
ax.set_xlabel("Week")
ax.set_ylabel("Total Basket Cost (Rs)")
ax.set_title(f"Basket Cost Trend (Last {RECENT_WEEKS} Weeks + Forecast)")
ax.legend()
ax.grid(alpha=0.3)
plt.tight_layout()
st.pyplot(fig)
...
if remaining_budget > 100:
    st.write(f"You have Rs {remaining_budget:.2f} left — consider adding more fruits or vegetables.")
else:
    st.write("Budget is nearly used up — the basket above is tight but on-target.")
...
if meal_protein < household_protein_goal:
    ...
    st.write("Protein goal met for this basket.")
