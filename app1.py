...
# Change the page tab title and main page header
st.set_page_config(page_title="Smart Grocery Predictor and Meal Planner", layout="wide")
st.title("🛒 Smart Grocery Predictor and Meal Planner")
...
def load_data():
  food_data = (
      pd.read_csv("food_dataset.csv").drop_duplicates().reset_index(drop=True)
  )
...
budget = st.sidebar.number_input(
    "Weekly grocery budget (Rs)", min_value=0.0, value=500.0, step=50.0
)
calorie_goal = st.sidebar.number_input(
    "Daily calorie goal per person (kcal)", min_value=0, value=2000, step=100
)
protein_goal = st.sidebar.number_input(
    "Daily protein goal per person (g)", min_value=0, value=60, step=5
)

st.sidebar.header("Personalization")
diet_pref = st.sidebar.selectbox("Dietary preference", ["any", "veg", "non-veg"])
exclude_raw = st.sidebar.text_input(
    "Foods to avoid (comma-separated)", value=""
)
household_size = st.sidebar.number_input(
    "Household size", min_value=1, value=1, step=1
)

st.sidebar.header("Forecast")
weeks_ahead = st.sidebar.slider(
    "Weeks ahead to forecast", min_value=1, max_value=8, value=3
)
...
if available.empty:
  st.error(
      "No foods left after filtering — loosen your exclusions or diet"
      " preference."
  )
  st.stop()
...
if spent > budget:
  over_budget_warning = (
      f"Just one item from each food group already costs Rs {spent:.2f}, which"
      f" is over your Rs {budget:.2f} budget. Showing the cheapest possible"
      " diverse plan anyway."
  )
else:
  remaining = available.drop(index=plan_idx).copy()
  remaining["Efficiency"] = (
      remaining["Calories"] / max(household_calorie_goal, 1)
      + remaining["Protein"] / max(household_protein_goal, 1)
  ) / remaining["Price"]
  remaining = remaining.sort_values("Efficiency", ascending=False)

  for idx, row in remaining.iterrows():
    if (
        got_calories >= household_calorie_goal
        and got_protein >= household_protein_goal
    ):
      break
...
future_weeks = pd.DataFrame(
    {"Week": np.arange(last_week + 1, last_week + 1 + weeks_ahead)}
)

price_models, model_names, future_prices = (
    {},
    {},
    {"Week": future_weeks["Week"].values},
)
...
for item in item_cols:
  ...
  best_model, best_name = (
      (lr, "LinearRegression") if lr_mae <= rf_mae else (rf, "RandomForest")
  )
  best_model.fit(X, y)
...
for item in item_cols:
  ...
  verdict = (
      "Buy now"
      if pct_change > 2
      else ("Wait" if pct_change < -2 else "No urgency")
  )
  advice_rows.append({
      "Item": item,
...
with left:
  fig, ax = plt.subplots()
  ax.pie(
      [meal_calories, meal_protein * 4],
      labels=["Calories (kcal)", "Protein (kcal-equiv)"],
      autopct="%1.1f%%",
...
for ax, item in zip(axes, item_cols):
  ...
  # Connect last actual observation directly to the forecast line
  connect_item = pd.concat([recent_history.tail(1), future_df])
  ax.plot(
      connect_item["Week"],
      connect_item[item],
      marker="o",
...
# ---------------- Zoomed-in Total Spend Chart ----------------
st.subheader("Total weekly cost forecast")
predicted_total = future_df["Total_Cost"].iloc[-1]
st.metric(
    f"Predicted total spend, week {int(future_df['Week'].iloc[-1])}",
    f"Rs {predicted_total:.2f}",
)

fig, ax = plt.subplots(figsize=(10, 4.5))
ax.plot(
    recent_history["Week"],
    recent_history["Total_Cost"],
    marker="o",
    markersize=5,
    color="#1f77b4",
    label=f"Actual cost (last {history_window} wks)",
)
connect_total = pd.concat([recent_history.tail(1), future_df])
ax.plot(
    connect_total["Week"],
    connect_total["Total_Cost"],
    marker="o",
    linestyle="--",
    color="red",
    linewidth=2,
    markersize=6,
    label=f"Forecast (next {weeks_ahead} wks)",
)

ax.set_xlabel("Week Number")
ax.set_ylabel("Total Cost (Rs)")
ax.set_title(f"Weekly Grocery Cost Trend & Forecast (Last {history_window} Weeks + Forecast)")
ax.legend()
...
if remaining_budget > 100:
  st.write(
      f"You have Rs {remaining_budget:.2f} left — consider adding more fruit"
      " or vegetables."
  )
else:
...
if meal_protein < household_protein_goal:
  st.write(
      "Protein is short of goal. Consider adding: Paneer, Eggs (if non-veg),"
      " Chickpeas, or Soybean Chunks."
  )
else:
...
if top_riser["Change %"] > 2:
  st.write(
      f"**{top_riser['Item']}** is trending up the most"
      f" (+{top_riser['Change %']}%) — buy it this week."
  )
else:
  st.write(
      "No item is trending sharply upward — no urgency to stock up on anything"
      " specific."
  )